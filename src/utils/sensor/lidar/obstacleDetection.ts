...
const isEligibleToBeACan = (matrix, row: number, column: number) => {
  return matrix[row][column] === cellStates.EMPTY;
};

const isTooCloseToWall = (matrix, row: number, column: number, depth: number = 1) => {
...
const canDetection = (matrix, pose, lidarData, resolution: number = 50) => {
